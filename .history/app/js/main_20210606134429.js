document.addEventListener('DOMContentLoaded', function (event) {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (element.closest('.js-drop-button')) {
      ...
      if (isActive) element.closest('.js-drop-item').classList.remove('active');
      else element.closest('.js-drop-item').classList.add('active');
    }
  ...
  document.addEventListener('mouseover', function (e) {
    ...
    if (item.closest('.svg-content path')) {
      ...
      let value = document
        .querySelector('.region__item[data-target=' + targetId + ']')
  ...
  if (generalImg) {
    ...
    if (buttonPrev) {
      ...
      var startPointX;
      var startPointY;
      document.querySelector('.js-general-image').addEventListener(
        'touchstart',
        function (event) {
          startPointX = event.changedTouches[0].screenX;
          startPointY = event.changedTouches[0].screenY;
